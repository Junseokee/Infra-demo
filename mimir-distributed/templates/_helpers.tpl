...
{{/*
Create the app name for clients. Defaults to the same logic as "policy.fullname", and default client expects "prometheus".
*/}}
{{- define "client.name" -}}
...
{{/*
The volume to mount for policy configuration
*/}}
...
{{- else -}}
checksum/config: {{ include (print .ctx.Template.BasePath "/policy-config.yaml") .ctx | sha256sum }}
{{- end }}
...
{{/*
policy.componentSectionFromName returns the sections from the user .Values in YAML
that corresponds to the requested component. policy.componentSectionFromName takes two arguments
  .ctx = the root context of the chart
  .component = the name of the component. policy.componentSectionFromName uses an internal mapping to know
                which component lives where in the values.yaml
Examples:
  $componentSection := include "policy.componentSectionFromName" (dict "ctx" . "component" "store-gateway") | fromYaml
...
{{- if not $componentSection -}}{{- printf "No component section mapping for %s not found in values; submit a bug report if you are a user, edit policy.componentSectionFromName if you are a contributor" .component | fail -}}{{- end -}}
{{- $section := .ctx.Values -}}
...
policy.vaultAgent.annotations takes 2 arguments
  .ctx = the root context of the chart
...
policy.siToBytes takes 1 argument
...
policy.parseCPU takes 1 argument
