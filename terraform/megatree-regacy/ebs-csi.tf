    ...
    Statement = [
      {
        ...
        Condition = {
          "StringEquals" = {
            "${replace(aws_eks_cluster.app_cluster.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:kube-system:ebs-csi-controller-sa",
...
data "tls_certificate" "recovery_cluster" {
  url = aws_eks_cluster.app_cluster.identity[0].oidc[0].issuer
}
...
resource "aws_eks_addon" "app_cluster_ebs_csi" {
